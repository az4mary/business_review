...
const growthPathCard = (path) => `
  <article class="path-card">
    ...
    <a href="${path.url}" data-event="growth_path_card_click" data-category="${path.id}">${path.ctaLabel || "View Products"} <span>${arrow}</span></a>
  </article>`;
...
export const renderHomePage = () => `
  ...
  <main id="main-content">
    ...
    <section class="section products" id="services"><div class="section-heading"><div><p class="eyebrow">Focused offers · Clear scope</p><h2>Start With a <em>Fixed-Price</em> Growth Service</h2></div><p>Choose a focused starting point with defined deliverables and a transparent price.</p></div><div class="product-grid">${products.map(productCard).join("")}</div></section>
