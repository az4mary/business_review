...
const galleryData = imageFiles.map((file, index) => ({
  label: galleryLabels[index] || `Property image ${index + 1}`,
  src: imageUrl(file)
}));
...
${header}
<main id="main-content">${main}${lightbox}</main>
${footer}
${script}
</body>
</html>`;
