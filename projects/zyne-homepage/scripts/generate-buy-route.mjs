...
.mobile-canvas{position:absolute;left:50vw;top:50%;width:941px;height:1672px;transform:translate(-50%,-50%) scale(var(--canvas-scale));transform-origin:center;overflow:hidden;background:#020303;padding:24px 30px 34px;color:#f2f2f0}
.mobile-page-one{font-family:"Inter Var",Inter,"Segoe UI",Arial,sans-serif}
.mobile-gallery-hero{position:relative;height:358px;border:1px solid #8a6900;border-radius:12px;overflow:hidden;background:#111}.mobile-gallery-hero img{width:100%;height:100%;object-fit:cover;display:block}.mobile-sale-badge{position:absolute;left:16px;top:10px;width:180px;height:120px;padding:18px 16px;color:#fff;background:#060606;border:2px solid #f2c400;clip-path:polygon(0 0,100% 0,100% 82%,50% 100%,0 82%)}.mobile-sale-badge strong{display:block;color:#f2cc00;font-size:24px;line-height:1.1;letter-spacing:.06em}.mobile-sale-badge span{display:block;margin-top:10px;font-size:16px;font-weight:800;line-height:1.35;letter-spacing:.04em}
...
.mobile-copy{padding:26px 16px 0}.mobile-copy h1{margin:0 0 8px;font-size:38px;line-height:1.05;letter-spacing:-.03em}.mobile-location{display:flex;align-items:center;gap:8px;margin:0 0 22px;color:#f3c45a;font-size:19px}.mobile-location svg{width:18px;height:18px;fill:#f3c45a}.mobile-price{margin:0 0 14px;color:#f3c45a;font-size:48px;font-weight:800;letter-spacing:-.035em}.mobile-description{margin:0;color:#f2f2f0;font-size:22px;line-height:1.45}
.mobile-metrics{margin:18px 16px 18px;display:grid;grid-template-columns:1fr 1fr;gap:10px}.mobile-metric{height:103px;border:1px solid #8a6900;border-radius:4px;display:grid;grid-template-columns:78px 1fr;align-items:center;padding:0 20px;background:#050606}.mobile-metric .icon{width:54px;height:54px;color:#f2cc00;stroke-width:1.2}.mobile-metric small{display:block;color:#f2cc00;font-size:16px;font-weight:800;letter-spacing:.06em}.mobile-metric strong{display:block;margin-top:8px;color:#f2f2f0;font-size:30px;line-height:1;font-weight:700}
.mobile-cta-stack{display:grid;gap:14px;margin:0 16px}.mobile-cta{height:63px;border-radius:4px;border:1px solid #a77d11;display:flex;align-items:center;justify-content:center;gap:14px;text-transform:uppercase;letter-spacing:.08em;font-size:20px;font-weight:800}.mobile-cta.primary{background:linear-gradient(180deg,#f8d374,#dda126);color:#070707}.mobile-cta.outline{background:#030303;color:#f2cc00}.mobile-cta .icon{width:28px;height:28px}
.mobile-tags{margin:22px 24px 16px;display:flex;flex-wrap:wrap;justify-content:center;gap:10px}.mobile-tags span{height:54px;padding:0 27px;border:1px solid #725600;border-radius:999px;display:inline-flex;align-items:center;gap:10px;color:#f2f2f0;font-size:22px}.mobile-tags .icon{width:26px;height:26px;color:#f2cc00}
.mobile-agent-card{margin:0 16px;border:1px solid #725600;border-radius:16px;padding:20px 30px;display:grid;grid-template-columns:154px 1fr;gap:24px;background:#050606}.mobile-agent-card img{width:154px;height:154px;object-fit:cover;border-radius:10px;border:1px solid #607080}.mobile-agent-copy h2{margin:0 0 8px;font-size:30px;line-height:1}.mobile-agent-copy em{display:inline-flex;align-items:center;gap:7px;margin-left:10px;padding:6px 12px;border-radius:4px;background:#5b6472;color:#fff;font-size:16px;font-style:normal;text-transform:uppercase;letter-spacing:.08em}.mobile-agent-copy p{margin:0;color:#e1ddd5;font-size:21px;line-height:1.45}.mobile-agent-actions{display:grid;grid-template-columns:1fr 1fr;gap:16px;margin-top:14px}.mobile-agent-actions a{height:52px;border:1px solid #c9a000;border-radius:4px;display:flex;align-items:center;justify-content:center;gap:10px;color:#f2cc00;text-transform:uppercase;font-size:18px;font-weight:800;letter-spacing:.08em}.mobile-agent-actions .icon{width:24px;height:24px}
@media(max-width:767px){.desktop-buy-pages{display:none}.mobile-buy-pages{display:block}.viewport-page{height:calc(100dvh - 92px)}.mobile-canvas{top:0;transform:translateX(-50%) scale(var(--canvas-scale));transform-origin:top center}.global-footer-spacer{display:none}}
...
const mobileMetric = (metric) => `<article class="mobile-metric">${icon(metric.icon)}<div><small>${metric.label}</small><strong>${metric.value}</strong></div></article>`;
  ...
  <main class="buy-page-stack mobile-buy-pages" aria-label="Mobile property listing">
    <section class="viewport-page mobile-page-one" aria-label="Mobile property overview">
      <div class="mobile-canvas">
        <button class="mobile-gallery-hero gallery-slot" type="button" data-photo-id="exterior" aria-label="Open gallery at front exterior"><img src="${photoById.get("exterior")?.src}" alt="${photoById.get("exterior")?.alt}"><span class="mobile-sale-badge"><strong>FOR SALE</strong><span>SELLER FINANCING AVAILABLE</span></span></button>
        ...
        <section class="mobile-copy"><h1>${listing.addressLine1} ${listing.addressLine2}</h1><p class="mobile-location"><svg viewBox="0 0 24 24" aria-hidden="true"><path d="M12 2a7 7 0 0 0-7 7c0 5.3 7 13 7 13s7-7.7 7-13a7 7 0 0 0-7-7Zm0 9.5A2.5 2.5 0 1 1 12 6a2.5 2.5 0 0 1 0 5.5Z"/></svg>${listing.location}</p><p class="mobile-price">${listing.price}</p><p class="mobile-description">${listing.description}</p></section>
        <section class="mobile-metrics">${listing.metrics.map(mobileMetric).join("")}</section>
...
const updateScale=()=>{const mobile=matchMedia("(max-width: 767px)").matches;const canvasWidth=mobile?941:1672;const canvasHeight=mobile?1672:941;document.documentElement.style.setProperty("--canvas-scale",String(Math.min(1,innerWidth/canvasWidth,innerHeight/canvasHeight)));};
